...
public class apsPreBolt extends BaseBasicBolt {
	...
	void streamPreFill() {
		...
		for (int j = 0; j < declrNum; ++j) {

			for (int k = 0; k < TopologyMain.winSize - 1; ++k) {

				newval = streamPreFill_val();

				strevec[j][k] = newval;

				curexp[j] = curexp[j] + newval / TopologyMain.winSize;
				cursqr[j] = cursqr[j] + newval * newval;
				cursum[j] = cursum[j] + newval;

				curdev[j] = cursqr[j] + TopologyMain.winSize * curexp[j]
						* curexp[j] - 2 * cursum[j] * curexp[j];
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		...
		declarer.declareStream("retriCommand", new Fields("command", "taskid"));
		...
		if (streType.compareTo("dataStre") == 0) {
			...
			for (i = 0; i < len; ++i) {
				if (ids.charAt(i) == ',') {
					...
					// outVectors = outVectors + prepStream2Str(retriTs,
					// streIdx);

					collector.emitDirect(task, "winStre", new Values(retriTs,
							tmpStr, prepStream2Str(retriTs, streIdx), taskId));

				}
			}

			collector.emit("retriCommand",
