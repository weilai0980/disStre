...
public class sigcoPreBolt extends BaseBasicBolt {
	...
	public void prepare(Map stormConf, TopologyContext context) {
		...
		tasks = context.getComponentTasks("llPre");// llPre
	...
	public double[] curexp = new double[declrNum + 10],
			...
			cursqrsum = new double[declrNum + 10],
			cursum = new double[declrNum + 10];

	public void prepNormalizedSW(int memidx) {
		int k = 0, cnt = 0;
		// tmpdim = 0;
	...
	public void idxNewTuple(int strid, double val, int flag) {
		...
		cursqrsum[tmpsn] = cursqrsum[tmpsn] - oldval * oldval * flag + newval
				* newval;
		cursum[tmpsn] = cursum[tmpsn] - oldval * flag + newval;

		curdev[tmpsn] = cursqrsum[tmpsn] + TopologyMain.winSize * curexp[tmpsn]
