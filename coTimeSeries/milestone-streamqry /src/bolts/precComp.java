...
public class precComp extends BaseBasicBolt {
...
//						}
					...
					if (j >= 1) {
						...
						double ncor=0.0, cor=0.0;
						k = vecst[j];
						while (k != veced[j]) {
							
							ncor+=((normvec[j-1][k]-normvec[j][k])*(normvec[j-1][k]-normvec[j][k]));
							cor+=((strevec[j-1][k]-curexp[j-1])*(strevec[j][k]-curexp[j])); 
						...
						ncor=(2.0-ncor)/2.0;
						cor=cor/(Math.sqrt(curdev[j-1])*Math.sqrt(curdev[j]));
						
						System.out.printf("timestamp %f:  normalized %f   direct %f  \n",curtstamp,ncor,cor);
