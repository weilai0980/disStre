	...
	public void execute(Tuple input, BasicOutputCollector collector) {
		...
		if (streType.compareTo("dataStre") == 0) {
			...
			if (ts - ststamp >= TopologyMain.winSize - 1) {
				...
				ststamp++;
				emisData = 0.0;
				int taskpos[]=new int[3],task=0, relPos=0;
				...
				for (i = 0; i < streidCnt; ++i) {
					...
					if (iniFlag == 1) {
						...
						if(task== preTaskCoor[i]-1)
						{
						
							collector.emit("streamData", new Values("vec", vecOutputStr,
									curtstamp, task, streid[i]));
							
							
							if(relPos== TopologyMain.taskCell)
							{
								collector.emit("streamData", new Values("pt", ptOutputStr,
										curtstamp, task+1, streid[i]));
							}
														
						}
						else if(task == preTaskCoor[i])
						{
							collector.emit("streamData", new Values("pt", ptOutputStr,
									curtstamp, task, streid[i]));
							
							if(relPos== TopologyMain.taskCell && preInTaskPos[i]==  TopologyMain.taskCell)
							{
								collector.emit("streamData", new Values("pt", ptOutputStr,
										curtstamp, task+1, streid[i]));
							}
							else if(relPos== TopologyMain.taskCell && preInTaskPos[i] !=  TopologyMain.taskCell)
							{
								collector.emit("streamData", new Values("vec", vecOutputStr,
										curtstamp, task+1, streid[i]));
							}
						...
						else if(task == preTaskCoor[i]+1)
						{
						   
							if(preInTaskPos[i]==  TopologyMain.taskCell)
							{
								collector.emit("streamData", new Values("pt", ptOutputStr,
										curtstamp, task, streid[i]));
							}
					...
					preTaskCoor[i] = taskpos[0];
					preInTaskPos[i]= taskpos[1];
